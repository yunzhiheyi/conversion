...
export default {
  ...
  computed: {
    ...
    blockStyle() {
      const res = this.vertical
        ? `top:${this.blockSize / 2}px;bottom:${this.blockSize / 2}px;width:${
            this.lineSize
          }px;left:50%;margin-left:-${this.lineSize / 2}px;background:${
            this.backgroundColor
          };border-radius:${this.lineSize / 2}px;`
        : `left:${this.blockSize / 2}px;right:${this.blockSize / 2}px;height:${
            this.lineSize
          }px;top:50%;margin-top:-${this.lineSize / 2}px;background:${
            this.backgroundColor
          };border-radius:${this.lineSize / 2}px;`;
      return res;
    },
    activeStyle() {
      const res = this.vertical
        ? `top:${this.left};bottom:${this.right};width:${
            this.lineSize
          }px;left:50%;margin-left:-${this.lineSize / 2}px;background:${
            this.activeColor
          };border-radius:${this.lineSize / 2}px;`
        : `left:${this.left};right:${this.right};height:${
            this.lineSize
          }px;top:50%;margin-top:-${this.lineSize / 2}px;background:${
            this.activeColor
          };border-radius:${this.lineSize / 2}px;`;
  ...
  methods: {
    selectorQuery() {
      ...
      query
        ...
        .boundingClientRect((data) => {
          ...
          const shift = +(
            ((that.val - that.min) / (this.max - this.min)) *
            (that.width - this.blockSize)
          ).toFixed(1);
    ...
    setVal(shift) {
      let val = +(
        (shift / (this.width - this.blockSize)) * (this.max - this.min) +
        this.min
      ).toFixed(0);
